    ...
    <el-container style="height: 50vw;">
      <el-aside width="370px">
        <div style="display: flex; margin-bottom: 1em;">
          ...
          <el-button type="primary" style="margin-left: 1.2em; font-size: 10px;" size="mini" @click="isAddModalOpen = true">
            新規テンプレート追加
          </el-button>
        </div>
        ...
        <div v-for="(tmp, i) in filteredTemplateQuestionsList" :key="i">
          ...
          <el-button class="card-button" @click="selectTemplateQuestions(tmp)">
            <el-card class="card-templates" :body-style="changeSelectedButtonColor(tmp.id)" shadow="never">
              <el-tag
                type="info"
                size="mini"
                effect="plain"
        ...
        <el-main>
          <el-card>
            ...
            <div class="template-buttons">
              <el-button v-if="showingTemplateQuestions.id" type="success" icon="el-icon-edit" @click="isEditModalOpen = true">編集する</el-button>
              <el-button v-if="showingTemplateQuestions.id" type="danger" icon="el-icon-deletes" @click="removeTemplateQuestions(showingTemplateQuestions)">削除する</el-button>
            </div>
          </el-card>
          ...
          <el-card style="margin: 10px 0;">
            <el-form>
              ...
              <el-form-item v-if="showingTemplateQuestions['en'] && showingTemplateQuestions['cn']" label="表示する質問の言語" style="text-align: left; margin-bottom: 10px;">
            ...
            <template v-if="showingQuestionType === 'descriptions'">
              <editable-questions :questions="showingTemplateQuestions[questionsLang].descriptions" :is-editable-questions="false" />
            </template>
            <template v-else>
              <editable-questions :questions="showingTemplateQuestions[questionsLang].selections" :is-editable-questions="false" />
            </template>
          </el-card>
        </el-main>
      </el-container>
    </el-container>
    <add-edit-template-questions :is-modal-open.sync="isAddModalOpen" :is-add="true" />
    <add-edit-template-questions :is-modal-open.sync="isEditModalOpen" :template-questions-data="showingTemplateQuestions" :is-add="false" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    filteredTemplateQuestionsList() {
      const categorizedQuestions = this.templateQuestionsList.filter(question => question.templateType === this.templateType)
      return this.searchText
        ? categorizedQuestions.filter(tmp => JSON.stringify(Object.values(tmp)).includes(this.searchText))
        : categorizedQuestions
    },
    descriptionsLength() {
      return this.showingTemplateQuestions[this.questionsLang].descriptions.length
    },
    selectionsLength() {
      return this.showingTemplateQuestions[this.questionsLang].selections.length
    },
    options: () => ({
  ...
  methods: {
    ...
    changeSelectedButtonColor(id) {
      if (id === this.showingTemplateQuestions.id) { return { 'background-color': '#e5e5ed' } }
    },
    // 表示内容をデフォルトテンプレートにする
    setDefaultTemplateQuestions() {
      this.showingTemplateQuestions = {
        name: 'デフォルトテンプレート',
        jp: this.templateType === 'rookie' ? getDefaultQuestionsForRookieByLang('jp') : getDefaultQuestionsByLang('jp'),
        en: this.templateType === 'rookie' ? getDefaultQuestionsForRookieByLang('en') : getDefaultQuestionsByLang('en'),
        cn: this.templateType === 'rookie' ? getDefaultQuestionsForRookieByLang('cn') : getDefaultQuestionsByLang('cn')
      }
    },
    // 質問テンプレートの削除
    async removeTemplateQuestions(tmp) {
      const confirm = await this.$confirm(`質問テンプレート「${tmp.name}」を削除してもよろしいですか`).catch(() => {})
