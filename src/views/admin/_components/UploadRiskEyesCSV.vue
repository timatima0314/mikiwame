  ...
  <div>
    <el-select v-model="selectedYYYYMM">
      <el-option
        ...
        :key="i"
        :label="month.format('YYYY年MM月分')"
        :value="month.format('YYYYMM')"
      />
    </el-select>
    <el-upload
      action=""
      :http-request="onUploadCSV"
      :multiple="false"
      accept="text/csv"
    ...
    <el-card
      header="直近3ヶ月のアップロード状況"
      style="margin-top: 1rem; width: 50%"
    >
      <el-table
        :data="last3monthsUploadStatus"
        :show-header="false"
        size="small"
      >
...
export default {
  ...
  methods: {
    async setLast3monthsUploadStatus() {
      this.last3monthsUploadStatus = await Promise.all(
        this.monthOptions.map(async (month) => {
          const snapshot = await getRiskEyesBillingRefByYYYYMM(
            month.format("YYYYMM")
          )
            .get()
    ...
    onUploadCSV({ file }) {
      ...
      this._parseCSV(file)
        .then(this._aggregateByID)
        .then(this._saveAggregation)
        .then(() => {
          this.$notify({
            type: "success",
            title: "Success",
        ...
        .catch((err) => {
          ...
          this.$notify({
            ...
            title: "Error",
            message:
              err.message && err.message.startsWith("CSV_PARSE_ERROR")
                ? "CSVの解析に失敗しました。CSVファイルの内容をお確かめください"
                : "CSVのアップロードに失敗しました。通信環境を確認したうえで再度お試しください。",
          });
        })
        .finally(() => {
      ...
      return new Promise((resolve, reject) => {
        Papa.parse(file, {
          ...
          complete: (result) => {
            ...
            if (firstHeader !== "利用者ID")
              reject(new Error(`CSV_PARSE_ERROR: invalid header`));
    ...
    _aggregateByID(rows) {
      ...
      rows.forEach((row) => {
        const [
          id /* 利用者ID */ /* 利用者名 */ /* 案件ID */ /* 調査種類 */ /* 案件区分 */ /* 案件名 */ /* 利用日 */,
          ,
          ,
          ,
          ,
