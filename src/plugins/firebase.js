...
if (process.env.VUE_APP_FUNCTIONS_EMULATOR_PORT) {
  functions.useFunctionsEmulator(
    `http://localhost:${process.env.VUE_APP_FUNCTIONS_EMULATOR_PORT}`
  );
}
export { functions };
...
export const adminConfigsRef = firebase.firestore().doc("admin_configs/1");
...
export const getRiskEyesBillingRefByYYYYMM = YYYYMM =>
  firebase.firestore().doc(`risk_eyes_billing/${YYYYMM}`);
