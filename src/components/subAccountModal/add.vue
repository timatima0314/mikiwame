<template>
  <el-dialog title="企業担当者の追加" :visible="isModalOpen" :before-close="handleClose" style="text-align: center" :width="$mq === 'pc' ? '60%' : '80%'" @close="close">
    <el-form
      ref="form"
      v-loading="loading"
      :model="form"
...
export default {
  ...
  methods: {
    ...
    async addSubAccount() {
      ...
      this.toggleLoading()
      const subAccountRef = await companiesCollectionRef.doc(this.companyId).collection('subAccounts')
      const snap = await subAccountRef.where('email', '==', this.form.email).get()
      if (!snap.empty) {
        this.$notify({
          type: 'error',
          title: 'Error',
      ...
      const newSubAccountData = await subAccountRef.add({ ...this.form, createdAt: new Date() })
        .catch(err => {
      ...
      functions.httpsCallable('notifySubAccountToSetPassword')({ companyId: this.companyId, subAccountId: newSubAccountData.id })
        .then(() => {
          this.$notify({
            title: 'Success',
