<template>
  <el-dialog :title="isAdd ? '新規テンプレートの追加' : '「' + templateQuestionsData.name + '」の編集'" :visible="isModalOpen" style="text-align: center" :width="$mq === 'pc' ? '60%' : '80%'" @close="close">
    <el-form ref="form" :model="form" :disabled="loading" :rules="rules">
      <el-form-item label="テンプレート名" prop="name">
        <el-input v-model="form.name" placeholder="質問のテンプレート名を入力してください" />
      </el-form-item>
      <el-form-item label="テンプレートタイプ" style="text-align: left; margin-bottom: 10px;">
      ...
      <el-form-item v-if="form['en'] && form['cn']" label="編集する質問の言語" style="text-align: left; margin-bottom: 10px;">
    ...
    <template v-if="showingQuestionType === 'descriptions'">
      <el-card>
        <editable-questions :questions="form[questionsLang] ? form[questionsLang].descriptions : []" :is-editable-questions="true" />
        <h3>独自の質問(必要に応じて追加してください)</h3>
        <el-input v-model="newQuestionText" type="textarea" placeholder="オリジナルの質問を入力してください" />
    ...
    <el-card v-else-if="showingQuestionType === 'selections'">
      <div v-for="(question, i) in form[questionsLang].selections" :key="i">
        <el-row type="flex" justify="space-between" align="middle">
          ...
          <el-col :span="2"><el-button icon="el-icon-lock" type="info" circle disabled /></el-col>
        </el-row>
    ...
    <el-button v-if="isAdd" type="primary" @click="addTemplateQuestions">保存する</el-button>
    <el-button v-else type="primary" @click="editTemplateQuestions">編集内容を保存する</el-button>
  </el-dialog>
</template>
...
export default {
  ...
  computed: {
    ...
    rules: () => ({
      name: [{ required: true, message: 'テンプレートの名前を入力してください', trigger: 'blur' }]
    }),
    options: () => ({
  ...
  methods: {
    ...
    resetQuestion() {
      if (this.isAdd) {
        this.form[this.questionsLang].descriptions = this.form.templateType === 'rookie'
          ? cloneDeep(getDefaultQuestionsForRookieByLang(this.questionsLang).descriptions)
          : cloneDeep(getDefaultQuestionsByLang(this.questionsLang).descriptions)
      } else {
        this.form[this.questionsLang].descriptions = this.templateQuestionsData[this.questionsLang]?.descriptions ?? []
      }
    },
    adaptQuestionToRank(label) {
      this.form[this.questionsLang] = label === 'rookie' ? getDefaultQuestionsForRookieByLang(this.questionsLang) : getDefaultQuestionsByLang(this.questionsLang)
    },
    // 新規質問テンプレートの追加
    async addTemplateQuestions() {
      const valid = await this.$refs.form.validate().catch(() => {})
      if (!valid) {
        this.$notify({ type: 'error', title: 'Error', message: '入力に不備がございます' })
        return
      }

      this.toggleLoading()
      companiesCollectionRef.doc(this.companyId).collection('templateQuestions').add({
        createdAt: new Date(),
        name: this.form.name,
        jp: this.form['jp'],
        en: this.form['en'],
