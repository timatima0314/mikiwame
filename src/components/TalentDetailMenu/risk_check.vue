  ...
  <div>
      ...
      <el-row :gutter="20">
        <el-col :span="7">
          <el-card class="card card-paper">
            <span class="title title-paper"><i class="el-icon-reading" /> 新聞</span>
            <div class="num">{{ paperClipCount }}件</div>
          </el-card>
        </el-col>
        <el-col :span="7">
          <el-card class="card card-web">
            <span class="title title-web"><i class="el-icon-monitor" /> web</span>
            <div class="num">{{ webClipCount }}件</div>
          </el-card>
        </el-col>
      </el-row>
      <div v-if="paperClipCount >= 1 || webClipCount >= 1" style="margin-left: 18px;">
        <el-button plain :loading="loading" type="primary" icon="el-icon-document" @click="openClipListPage">
          記事一覧
        </el-button>
        <p style="font-size: 0.8rem;">※推奨ブラウザ：Chrome,Firefox（現段階ではこちらを推奨します）</p>
        <p style="font-weight: bold; font-size: 1.3rem;">記事一覧表示用ID: {{ riskEyesId }}</p>
      </div>
    ...
    <div v-else>
      ...
      >
        ...
        <el-button slot="reference" :loading="loading" style="margin-top: 5px;" @click="onClickGetClipCount">
          件数取得
        </el-button>
      </el-popover>
    </div>
...
export default {
  ...
  methods: {
    ...
    onClickGetClipCount() {
      ...
      functions.httpsCallable('fetchAndSaveRiskCheck')({ companyId: this.companyId, talentId })
        .then(response => {
          ...
          if (err.code === 'permission-denied') {
            ...
            this.$notify({
              type: 'error',
              title: 'Error',
              message: '記事件数の取得に失敗しました。時間をおいて再度お試しください'
            })
          }
    ...
    openClipListPage() {
      ...
      functions.httpsCallable('getRiskCheckResultUrl')({ talentName: name })
        .then(response => {
          ...
          this.$notify({
            type: 'error',
            title: 'Error',
            message: '記事のURLの取得に失敗しました。時間をおいて再度お試しください'
          })
        }).finally(() => { this.loading = false })
    }
  }
