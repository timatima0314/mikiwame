<template>
  <el-form v-model="form" v-loading="loading" class="container" label-width="9rem" :model="form" :rules="rules">
    <h1>カード設定</h1>
    <el-form-item prop="cardNumber" label="カード番号">
      <el-input :value="form.cardNumber" :maxlength="cardNumberInputMaxLength" placeholder="1234 1234 1234 1234" @input="addSpaceToCardNumber" />
    </el-form-item>
    <el-form-item prop="cardNumber" label="カード名義人">
      <el-input v-model="form.name" :maxlength="cardNumberInputMaxLength" placeholder="TARO YAMADA" />
    </el-form-item>
    <el-form-item prop="" label="有効期限">
      <el-select v-model="form.expireMonth" placeholder="月">
        <el-option v-for="month in months" :key="`month${month}`" :label="month" :value="month" />
      </el-select>
      <el-select v-model="form.expireYear" placeholder="年">
        <el-option v-for="year in years" :key="`year${year}`" :label="year" :value="year" />
      </el-select>
    </el-form-item>
    <el-form-item label="セキュリティコード" prop="securityCode">
      <el-input type="number" minlength="3" maxlength="4" placeholder="123" style="width: 5rem" :value="form.securityCode" @input="onInputSecurityCode" />
    </el-form-item>
    <Notice :agree.sync="agree" style="margin-bottom: 1rem" />
    <div style="text-align: center;">
      <el-button type="primary" :disabled="!agree" @click="onClickSave">登録</el-button>
    </div>
  </el-form>
</template>
...
export default {
  ...
  computed: {
    ...
    rules: () => ({
      securityCode: [{ pattern: /^\d{3,4}$/, trigger: 'blur', message: 'セキュリティコードは3~4文字で入力してください' }]
    }),
    months: () => range(1, 12 + 1).map(month => String(month).padStart(2, '0')), // e.g. ['01', '02', ...]
    years: () => {
  ...
  methods: {
    ...
    async onClickSave() {
      ...
      if (!this.agreeInDB && this.agree) {
        ...
        updateCompany({ companyId: this.companyId, data: { isAgreeToNotice: true }}).catch(() => {})
      }

      try {
        // コールバック関数名を文字列で渡す必要がある。そのため、windowオブジェクトに関数を格納
        window.setTokenizeResult = result => { this.tokenizeResult = result } // e.g. tokenizeResult: { resultCode: '000', tokenObject: { token: 'tokentoken', ... } }
        ...
        await functions.httpsCallable('createOrUpdateCreditCard')({ companyId: this.companyId, token, maskedCardNo })
        this.$notify({
          type: 'success',
          title: 'Success',
        ...
        this.$notify({
          type: 'error',
          title: 'Error',
          message: err.message === 'INVALID_CARD'
            ? '入力されたカードが有効ではありません'
            : 'カードの登録に失敗しました。時間をおいて再度お試しください'
        })
      } finally {
        this.loading = false
      }
