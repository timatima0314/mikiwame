  ...
  >
    ...
    >
      ...
      <el-form-item label="質問テンプレート" style="text-align: left">
        <el-select
          v-model="templateQuestionsId"
          placeholder="Select"
          :disabled="isEditableQuestions"
          style="padding-right: 10px"
          @change="questionsLang = 'jp'"
        >
          <el-option
        ...
        <el-button
          v-if="!isEditableQuestions"
          type="primary"
          @click="confirmTemplateQuestions"
          >このテンプレートをもとに質問を作成</el-button
        >
        <el-button v-else type="primary" @click="changeTemplateQuestions"
          >テンプレートを変更</el-button
        >
      </el-form-item>
      ...
      <el-form-item
        v-if="templateQuestions['en'] && templateQuestions['cn']"
        label="編集する質問の言語"
        style="text-align: left"
      >
        ...
        <p style="margin-top: 0px; font-size: 0.7rem">
          ※その国の言語で質問した場合、その国の言語で返答されます
        </p>
      </el-form-item>
      ...
      <template v-if="showingQuestionType === 'descriptions'">
        <el-card>
          <editable-questions
            :questions="
              isEditableQuestions
                ? questions[questionsLang].descriptions
                : templateQuestions[questionsLang].descriptions
            "
            :is-editable-questions="isEditableQuestions"
          />
      ...
      <el-card v-else-if="showingQuestionType === 'selections'">
        <div
          v-for="(question, i) in isEditableQuestions
            ? questions[questionsLang].selections
            : templateQuestions[questionsLang].selections"
          :key="i"
        >
          <el-row type="flex" justify="space-between" align="middle">
            ...
            >
            <el-col v-if="isEditableQuestions" :span="2"
...
export default {
  ...
  computed: {
    ...
    templateQuestions() {
      return this.templateQuestionsList.find(
        (tmp) => tmp.id === this.templateQuestionsId
      );
    },
    filteredTemplateQuestionList() {
      return this.templateQuestionsList.filter(
        (question) => question.templateType === this.form.rank
      );
    },
    descriptionsLength() {
      return this.isEditableQuestions
        ? this.questions[this.questionsLang].descriptions.length
        : this.templateQuestions[this.questionsLang].descriptions.length;
    },
    selectionsLength() {
      return this.isEditableQuestions
        ? this.questions[this.questionsLang].selections.length
        : this.templateQuestions[this.questionsLang].selections.length;
    },
    rules: () => ({
      name: [
      ...
      currentCompany: [
        { required: true, message: "会社名を入力して下さい", trigger: "blur" },
      ],
      currentJobCategory: [
  ...
  methods: {
    ...
    // 質問をデフォルトに戻す
    resetQuestion() {
      this.questions[this.questionsLang].descriptions = cloneDeep(
        this.templateQuestions[this.questionsLang].descriptions
      );
    },
    // 質問テンプレート決定
    ...
    async changeTemplateQuestions() {
      ...
      const confirm = await this.$confirm(
        `質問テンプレートを変更した場合，編集した質問内容は保存されませんがよろしいですか？`
      ).catch(() => {});
    ...
    adaptQuestionIdToRank(label) {
      this.templateQuestionsId =
        label === "rookie" ? "rookieDefault" : "fulltimeDefault";
    },
    async addTalent() {
      ...
      if (!this.isEditableQuestions)
        this.questions = cloneDeep(this.templateQuestions);

      this.toggleLoading();
      if (!this.isBulk) {
        const questions = this.isEditableQuestions
          ? this.questions
          ...
          this.talents.map((talent) =>
            companiesCollectionRef
              ...
              .add({
                ...
                status: statusProperty.talentRegistered.key,
                refereeBreakdown: this.form.refereeBreakdown.map(
                  (breakdown) => ({
                    ...breakdown,
                    isRelationshipSelectedByCompany: Boolean(
